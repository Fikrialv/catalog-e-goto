    ...
    <section className="mt-10 grid gap-8 xl:grid-cols-[0.85fr_1.15fr]">
      <form onSubmit={create} className="rounded-[1.5rem] border border-ink/10 bg-primary-soft p-6 sm:p-7">
        ...
        <div className="mt-6 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3"><label className="block text-sm font-bold text-ink">Potongan (Rp)<input required min="1000" type="number" value={amount} onChange={(event) => setAmount(event.target.value)} className="mt-2 min-h-11 w-full rounded-xl border border-ink/15 bg-white px-3 outline-none" /></label><label className="block text-sm font-bold text-ink">Limit pakai<input required min="1" type="number" value={usageLimit} onChange={(event) => setUsageLimit(event.target.value)} className="mt-2 min-h-11 w-full rounded-xl border border-ink/15 bg-white px-3 outline-none" /></label></div>
        </div>
      ...
      <section className="rounded-[1.5rem] border border-ink/10 bg-white p-6 sm:p-7">
        <div className="flex items-end justify-between gap-4"><div><p className="eyebrow">Voucher inventory</p><h2 className="mt-2 font-serif text-3xl tracking-[-0.04em] text-ink">Kode aktif</h2></div><span className="text-sm font-semibold text-ink-muted">{vouchers.length} kode</span></div>
